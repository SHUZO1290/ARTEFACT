...
function renderTopbar() {
  ...
  const right = u ? `
    ...
    ${u.isAdmin ? `<button class="btn btn-ghost btn-sm nav-icon-btn" id="admin-nav-btn" title="Админ-панель" style="color:var(--gold)">${I.bolt}</button>` : ''}
    <div class="wallet-widget">
      <button class="wallet wallet-btn" id="wallet-btn">
        <span class="diamond">◈</span><span id="wallet-bal">${fmt(u.balance)}</span>
      </button>
      <div class="wallet-pop" id="wallet-pop">
        <div class="wallet-pop-head">
          <div class="wallet-user-pill">
            ${avatarEl(u.avatarSeed)}
            <div class="wallet-user-info">
              <div class="wallet-username">${esc(u.username)}</div>
              <div class="wallet-sub">Профиль пользователя</div>
            </div>
          </div>
        </div>
        <div class="wallet-balance-block">
          <div class="wallet-balance-label">Баланс</div>
          <div class="wallet-balance-val"><span class="diamond">◈</span> <span id="wallet-pop-bal">${fmt(u.balance)}</span> <span class="wallet-rate">≈ ${fmt(u.balance)} ₽</span></div>
          <div class="wallet-rate-note">1 токен = 1 рубль</div>
        </div>
        <div class="wallet-actions">
          <button class="wallet-action-btn" id="wpop-deposit">${I.plus}<span>Пополнить</span></button>
          <button class="wallet-action-btn" id="wpop-withdraw">${I.send}<span>Вывести</span></button>
          <button class="wallet-action-btn" id="wpop-transfer">${I.swap}<span>Перевести</span></button>
        </div>
      </div>
    </div>
    <div class="umenu">
  ...
  if (adminBtn) adminBtn.onclick = () => { location.hash = '#/admin'; };
  ...
  if (walletBtn) {
    walletBtn.onclick = (e) => {
      e.stopPropagation();
      const pop = document.getElementById('wallet-pop');
      if (pop) pop.classList.toggle('open');
      document.getElementById('umenu-pop')?.classList.remove('open');
    };
    document.addEventListener('click', () => { document.getElementById('wallet-pop')?.classList.remove('open'); });
  }
  const wDeposit = document.getElementById('wpop-deposit');
  if (wDeposit) wDeposit.onclick = (e) => { e.stopPropagation(); document.getElementById('wallet-pop')?.classList.remove('open'); openDepositModal(); };
  const wWithdraw = document.getElementById('wpop-withdraw');
  if (wWithdraw) wWithdraw.onclick = (e) => { e.stopPropagation(); document.getElementById('wallet-pop')?.classList.remove('open'); openWithdrawModal(); };
  const wTransfer = document.getElementById('wpop-transfer');
  if (wTransfer) wTransfer.onclick = (e) => { e.stopPropagation(); document.getElementById('wallet-pop')?.classList.remove('open'); openTransferModal(); };
...
/* ──────────────────────────────────────────────────────────────
   18б. КОШЕЛЁК: ПОПОЛНЕНИЕ
   ────────────────────────────────────────────────────────────── */

function openDepositModal() {
  if (!requireAuth()) return;
  let step = 'method'; // 'method' → 'details' → 'qr' → 'done'
  let chosenMethod = null;
  let chosenAmount = 0;

  const close = openModal(`
    <h2 style="margin-bottom:.2rem">Пополнить баланс</h2>
    <p class="sub" style="margin-bottom:1.2rem">1 токен = 1 рубль. Выберите способ пополнения:</p>
    <div class="wallet-method-list">
      <button class="wallet-method-item" data-method="card">
        <span class="method-icon">💳</span>
        <span>Карта РФ</span>
      </button>
      <button class="wallet-method-item" data-method="qr">
        <span class="method-icon">📱</span>
        <span>QR-код</span>
      </button>
      <button class="wallet-method-item" data-method="phone">
        <span class="method-icon">📞</span>
  ...
  function showDetails(method) {
    chosenMethod = method;
    const labels = { card: 'Карта РФ', qr: 'QR-код', phone: 'Номер телефона' };
    let extraField = '';
    if (method === 'phone') extraField = `<div class="form-group" style="margin-top:.8rem"><label>Номер телефона</label><input type="tel" id="dep-phone" placeholder="+7 (999) 999-99-99" style="width:100%"></div>`;
    modalBody.innerHTML = `
      <h2 style="margin-bottom:.2rem">Пополнить • ${labels[method]}</h2>
      <p class="sub" style="margin-bottom:1.2rem">Курс: 1 токен = 1 ₽</p>
      <div class="form-group">
        <label>Сумма (₽ = токены)</label>
        <input type="number" id="dep-amount" min="1" max="1000000" placeholder="Введите сумму…" style="width:100%">
      </div>
      ${extraField}
      <button class="btn btn-primary" id="dep-confirm" style="width:100%;margin-top:1rem">Подтвердить</button>
      <p class="sub" style="margin-top:.6rem;text-align:center">Нажмите «Подтвердить» — средства зачислятся на баланс</p>
    `;
    document.getElementById('dep-confirm').onclick = async () => {
      const amount = parseInt(document.getElementById('dep-amount').value);
      if (!amount || amount < 1) { toast('Укажите сумму', 'err'); return; }
      chosenAmount = amount;
      if (method === 'qr') {
        showQR(amount);
      } else {
        await doDeposit(amount, method === 'phone' ? 'phone' : 'card');
      }
    };
  }

  function showQR(amount) {
    // Simple QR template using a public QR generator
    const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent('ARTEFACT_DEPOSIT_' + amount + '_RUB_SUCCESS')}`;
    modalBody.innerHTML = `
      <h2 style="margin-bottom:.5rem">Сканируйте QR-код</h2>
      <p class="sub" style="margin-bottom:1rem">Сумма: <b>${fmt(amount)} ₽</b> → <b>${fmt(amount)} токенов</b></p>
      <div style="display:flex;justify-content:center;margin:1rem 0">
        <img src="${qrUrl}" alt="QR" style="border-radius:12px;border:1px solid var(--line);width:200px;height:200px">
      </div>
      <p style="text-align:center;color:var(--text-3);font-size:.85rem;margin-bottom:1rem">После сканирования вы увидите подтверждение оплаты</p>
      <button class="btn btn-primary" id="dep-qr-continue" style="width:100%">✓ Продолжить (оплата выполнена)</button>
    `;
    document.getElementById('dep-qr-continue').onclick = () => doDeposit(amount, 'qr');
  }

  async function doDeposit(amount, method) {
    try {
      const { balance, credited } = await API.post('/wallet/deposit', { amount, method });
      ...
      modalBody.innerHTML = `
        <div style="text-align:center;padding:1.5rem 0">
          ...
          <h2>Пополнение успешно!</h2>
          <p class="sub">На ваш счёт зачислено <b>${fmt(credited)} токенов</b></p>
          <p style="margin-top:.5rem;color:var(--gold);font-weight:700;font-size:1.1rem">Баланс: ◈ ${fmt(balance)}</p>
          <button class="btn btn-primary" style="margin-top:1.2rem" id="dep-done">Готово</button>
        </div>
      `;
      document.getElementById('dep-done').onclick = close;
      toast(`Зачислено ${fmt(credited)} токенов`, 'ok');
    } catch (e) { toast(e.message, 'err'); }
  }
  ...
  setTimeout(() => {
    document.querySelectorAll('[data-method]').forEach(btn => {
      btn.onclick = () => showDetails(btn.getAttribute('data-method'));
    });
  }, 50);
}
...
function openWithdrawModal() {
  if (!requireAuth()) return;
  const close = openModal(`
    <h2 style="margin-bottom:.2rem">Вывести средства</h2>
    <p class="sub" style="margin-bottom:1.2rem">Выберите способ вывода:</p>
    <div class="wallet-method-list">
      <button class="wallet-method-item" data-wmethod="card_rf">
        <span class="method-icon">💳</span>
        <span>Карта РФ</span>
      </button>
      <button class="wallet-method-item" data-wmethod="crypto">
        <span class="method-icon">₿</span>
        <span>Криптовалюта</span>
      </button>
      <button class="wallet-method-item" data-wmethod="ptp">
        <span class="method-icon">🤝</span>
        <span>P2P</span>
      </button>
    </div>
  ...
  function showWithdrawForm(method) {
    const labels = { card_rf: 'Карта РФ', crypto: 'Криптовалюта', ptp: 'P2P' };
    let detailField = '';
    if (method === 'card_rf') detailField = `<div class="form-group" style="margin-top:.8rem"><label>Номер карты</label><input type="text" id="wdraw-detail" placeholder="0000 0000 0000 0000" style="width:100%"></div>`;
    else if (method === 'crypto') detailField = `<div class="form-group" style="margin-top:.8rem"><label>Адрес кошелька</label><input type="text" id="wdraw-detail" placeholder="Адрес USDT/BTC…" style="width:100%"></div>`;
    else if (method === 'ptp') detailField = `<div class="form-group" style="margin-top:.8rem"><label>Никнейм на P2P-площадке</label><input type="text" id="wdraw-detail" placeholder="Ваш никнейм…" style="width:100%"></div>`;

    modalBody.innerHTML = `
      <h2 style="margin-bottom:.2rem">Вывод • ${labels[method]}</h2>
      <p class="sub" style="margin-bottom:1rem">Баланс: <b>◈ ${fmt(state.user.balance)}</b> | 1 токен = 1 ₽</p>
      <div class="form-group">
        <label>Сумма (токены = ₽)</label>
        <input type="number" id="wdraw-amount" min="1" max="${state.user.balance}" placeholder="Введите сумму…" style="width:100%">
      </div>
      ${detailField}
      <button class="btn btn-primary" id="wdraw-confirm" style="width:100%;margin-top:1rem">Вывести</button>
    `;
    document.getElementById('wdraw-confirm').onclick = async () => {
      const amount = parseInt(document.getElementById('wdraw-amount').value);
      if (!amount || amount < 1) { toast('Укажите сумму', 'err'); return; }
      try {
        const { balance, withdrawn } = await API.post('/wallet/withdraw', { amount, method });
        state.user.balance = balance;
        localStorage.setItem('artefact_user', JSON.stringify(state.user));
        updateWalletDisplay(balance);
        modalBody.innerHTML = `
          <div style="text-align:center;padding:1.5rem 0">
            <div style="font-size:3rem;margin-bottom:.5rem">✅</div>
            <h2>Вывод успешен!</h2>
            <p class="sub"><b>${fmt(withdrawn)} токенов</b> отправлены на ваши реквизиты</p>
            <p style="margin-top:.5rem;color:var(--text-3);font-size:.85rem">Обработка обычно занимает до 24 часов</p>
            <p style="margin-top:.3rem;color:var(--gold);font-weight:700">Баланс: ◈ ${fmt(balance)}</p>
            <button class="btn btn-primary" style="margin-top:1.2rem" id="wdraw-done">Готово</button>
          </div>
        `;
        document.getElementById('wdraw-done').onclick = close;
        toast(`Вывод ${fmt(withdrawn)} токенов выполнен`, 'ok');
      } catch (e) { toast(e.message, 'err'); }
    };
  }

  setTimeout(() => {
    document.querySelectorAll('[data-wmethod]').forEach(btn => {
      btn.onclick = () => showWithdrawForm(btn.getAttribute('data-wmethod'));
    });
  }, 50);
}
...
function openTransferModal() {
  if (!requireAuth()) return;
  const close = openModal(`
    <h2 style="margin-bottom:.5rem">Перевести токены</h2>
    <p class="sub" style="margin-bottom:1.2rem">Баланс: <b>◈ ${fmt(state.user.balance)}</b> | 1 токен = 1 ₽</p>
    <div class="form-group">
      <label>Никнейм получателя</label>
      <input type="text" id="tr-username" placeholder="Имя пользователя…" style="width:100%">
    </div>
    <div class="form-group" style="margin-top:.8rem">
      <label>Сумма (токены)</label>
      <input type="number" id="tr-amount" min="1" max="${state.user.balance}" placeholder="Сколько перевести…" style="width:100%">
    </div>
    <button class="btn btn-primary" id="tr-confirm" style="width:100%;margin-top:1rem">Перевести</button>
  `, true);

  const modalBody = document.querySelector('.modal');
  document.getElementById('tr-confirm').onclick = async () => {
    const toUsername = document.getElementById('tr-username').value.trim();
    const amount = parseInt(document.getElementById('tr-amount').value);
    if (!toUsername) { toast('Укажите никнейм получателя', 'err'); return; }
    if (!amount || amount < 1) { toast('Укажите сумму', 'err'); return; }
    try {
      const { balance, transferred, to } = await API.post('/wallet/transfer', { toUsername, amount });
      state.user.balance = balance;
      localStorage.setItem('artefact_user', JSON.stringify(state.user));
      updateWalletDisplay(balance);
      modalBody.innerHTML = `
        <div style="text-align:center;padding:1.5rem 0">
          ...
          <p class="sub"><b>${fmt(transferred)} токенов</b> переведены пользователю <b>${esc(to)}</b></p>
          <p style="margin-top:.5rem;color:var(--gold);font-weight:700">Ваш баланс: ◈ ${fmt(balance)}</p>
          <button class="btn btn-primary" style="margin-top:1.2rem" id="tr-done">Готово</button>
        </div>
      `;
      document.getElementById('tr-done').onclick = close;
      toast(`Переведено ${fmt(transferred)} токенов → ${to}`, 'ok');
    } catch (e) { toast(e.message, 'err'); }
  };
}

function updateWalletDisplay(balance) {
  const el1 = document.getElementById('wallet-bal');
  const el2 = document.getElementById('wallet-pop-bal');
  if (el1) el1.textContent = fmt(balance);
  if (el2) el2.textContent = fmt(balance);
  renderTopbar();
}
...
async function viewAdmin() {
  if (!state.user || !state.user.isAdmin) {
    app().innerHTML = `<div class="container"><div class="empty">${I.info}<h3>Нет доступа</h3><p>Этот раздел только для администраторов.</p></div></div>`;
    return;
  }

  const { tickets } = await API.get('/support/admin');
  const open = tickets.filter(t => t.status !== 'closed');
  const closed = tickets.filter(t => t.status === 'closed');

  app().innerHTML = `<section class="section-pad"><div class="container" style="max-width:900px">
    <div class="section-head">
      <div>
        <span class="eyebrow">Администратор</span>
        <h2>Панель управления</h2>
        <p>Ответы на обращения пользователей. Открытых: <b>${open.length}</b>, закрытых: <b>${closed.length}</b></p>
      </div>
    </div>
    <div id="admin-tickets">
      ${tickets.length === 0 ? emptyBox('Нет обращений', 'Пользователи пока не написали в поддержку.') : ''}
      ${open.length > 0 ? `<h3 style="margin-bottom:1rem;color:var(--gold)">Открытые (${open.length})</h3>` : ''}
      ${open.map(t => adminTicketCard(t)).join('')}
      ${closed.length > 0 ? `<h3 style="margin:1.5rem 0 1rem;color:var(--text-3)">Закрытые (${closed.length})</h3>` : ''}
      ${closed.map(t => adminTicketCard(t)).join('')}
    </div>
  </div></section>`;

  document.querySelectorAll('[data-reply-ticket]').forEach(btn => {
    btn.onclick = () => openAdminReplyModal(btn.getAttribute('data-reply-ticket'));
  });
  document.querySelectorAll('[data-close-ticket]').forEach(btn => {
    btn.onclick = async () => {
      if (!confirm('Закрыть обращение?')) return;
      try {
        await API.post('/support/' + btn.getAttribute('data-close-ticket') + '/close', {});
...
function adminTicketCard(t) {
  const lastMsg = t.messages[t.messages.length - 1];
  const isClosed = t.status === 'closed';
  return `<div class="admin-ticket-card ${isClosed ? 'closed' : ''}">
    <div class="admin-ticket-head">
      <div class="admin-ticket-user">
        ${avatarEl(t.user?.avatarSeed || 0, 'sm')}
        <div>
          <div class="admin-ticket-username">${esc(t.user?.username || 'Неизвестно')}</div>
          <div class="admin-ticket-time">${timeAgo(t.createdAt)} · ${t.messages.length} сообщ. · <span class="badge ${isClosed ? '' : 'r-rare'}">${isClosed ? 'Закрыт' : 'Открыт'}</span></div>
        </div>
      </div>
      ${!isClosed ? `<div style="display:flex;gap:.5rem">
        <button class="btn btn-outline btn-sm" data-reply-ticket="${t.id}">Ответить</button>
        <button class="btn btn-ghost btn-sm" data-close-ticket="${t.id}">Закрыть</button>
      </div>` : ''}
    </div>
    <div class="admin-ticket-msgs">
      ${t.messages.slice(-3).map(m => `
        <div class="admin-ticket-msg ${m.from === 'admin' ? 'admin' : 'user'}">
          <span class="admin-msg-who">${m.from === 'admin' ? '🛡 Поддержка' : esc(t.user?.username || 'User')}</span>
          <span class="admin-msg-text">${esc(m.text.slice(0, 200))}</span>
          <span class="admin-msg-time">${timeAgo(m.createdAt)}</span>
        </div>`).join('')}
      ${t.messages.length > 3 ? `<div style="color:var(--text-3);font-size:.8rem;padding:.3rem 0">…ещё ${t.messages.length - 3} сообщ.</div>` : ''}
    </div>
  </div>`;
}
...
function openAdminReplyModal(ticketId) {
  const close = openModal(`
    <h2 style="margin-bottom:.5rem">Ответить на обращение</h2>
    <textarea id="admin-reply-text" rows="4" placeholder="Напишите ответ пользователю…" style="width:100%;resize:vertical;min-height:100px"></textarea>
    <button class="btn btn-primary" id="admin-reply-send" style="width:100%;margin-top:.8rem">Отправить ответ</button>
  `, true);

  document.getElementById('admin-reply-send').onclick = async () => {
    const text = document.getElementById('admin-reply-text').value.trim();
    if (!text) { toast('Введите текст ответа', 'err'); return; }
    try {
      await API.post('/support/' + ticketId + '/reply', { text });
