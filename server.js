...
function publicUser(u) {
  if (!u) return null;
  return { id: u.id, username: u.username, balance: u.balance, avatarSeed: u.avatarSeed, bio: u.bio || '', createdAt: u.createdAt };
}
...
app.get('/api/support/admin', auth, (req, res) => {
  const isAdmin = req.user.username === 'admin' || db.users[0]?.id === req.user.id;
